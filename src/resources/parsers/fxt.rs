...
pub fn parse_fxt(data: &[u8]) -> Result<TextBundle> {
    ...
    while current_pos < decrypted.len() {
        ...
        if !entry_data.is_empty() {
            ...
            if let Ok((_, (key, value))) = parse_entry_nom(&s) {
...
mod tests {
    ...
    #[test]
    fn test_parse_fxt() {
        let mut data = vec![0xbf, 0xf8, 0xbd, 0x49, 0x62, 0xbe, 0x02, 0xef];
        data.extend_from_slice(b"swer the phone");
        for i in 8..data.len() { data[i] = data[i].wrapping_add(1); }
        data.push(1);
