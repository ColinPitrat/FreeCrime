...
The `newcarclut` section contains both cars remap and pedestrian remaps. There
are 12 remaps per car model, in the order they appear in `car_info`. Even if a
car doesn't have any remap or has less than 12, the 12 remaps will be present
(the unused ones will be identical to the no-remap palette).
These remaps are close, but not perfectly identical, to the ones obtained by
applying the hls remap from the car_info section (for example in STYLE001.G24,
the Bug has a dark-orange hls remap which is not present in the clut and the 4x4
has 10 hls remaps but only 8 modified palettes).

Just after the cars remap, there are 64 pedestrian remaps. These are, therefore,
the 64 last remaps of the section.
